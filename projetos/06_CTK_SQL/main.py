...
def selectByMes(mes):
    ...
    try:
        ...
        query = """
            SELECT id, titulo, valor, metodo_pagamento, dia, mes, ano
            FROM vendas
            WHERE mes = %s
            ORDER BY ano DESC, dia DESC, id DESC
        """
        cursor.execute(query, (mes,))
        return cursor.fetchall()
    except Error as error:
        raise RuntimeError(f"Erro ao buscar vendas: {error}") from error
    finally:
        if cursor is not None:
            cursor.close()
        if conn is not None and conn.is_connected():
...
def render_sales_results(parent, mes, sales):
    for widget in parent.winfo_children():
        widget.destroy()

    header = ctk.CTkLabel(parent, text=f"Resultados de {mes}", font=ctk.CTkFont(size=18, weight="bold"))
    header.pack(anchor="w", padx=12, pady=(10, 8))

    if not sales:
        empty_label = ctk.CTkLabel(parent, text="Nenhuma venda encontrada para este mes.")
        empty_label.pack(anchor="w", padx=12, pady=(0, 10))
        return

    for sale in sales:
        card = ctk.CTkFrame(parent, fg_color="#4f5252")
        card.pack(fill="x", padx=12, pady=6)

        valor = float(sale["valor"])
        text = (
            f"ID: {sale['id']} | {sale['titulo']}\n"
            f"Valor: R$ {valor:.2f} | Metodo: {sale['metodo_pagamento']}\n"
            f"Data: {int(sale['dia']):02d}/{sale['mes']}/{sale['ano']}"
        )
        ctk.CTkLabel(card, text=text, justify="left").pack(anchor="w", padx=10, pady=8)
...
def create_month_buttons(parent, on_month_click):
    months = [
        ("Janeiro", "janeiro"),
        ("Fevereiro", "fevereiro"),
        ("Marco", "marco"),
    ...
    for col, month in enumerate(months):
        ...
        month_label, month_value = month
        month_button = ctk.CTkButton(
            parent,
            text=month_label,
            command=lambda m=month_value: on_month_click(m),
            fg_color="#000000",
            hover_color="#1a1a1a",
            text_color="#ffffff",
        )
...
def main():
    ...
    right_col_content = ctk.CTkScrollableFrame(right_col, fg_color="transparent")
    right_col_content.pack(fill="both", expand=True, padx=8, pady=8)

    def on_month_click(mes):
        try:
            sales = selectByMes(mes)
            render_sales_results(right_col_content, mes, sales)
        except RuntimeError as error:
            render_sales_results(right_col_content, mes, [])
            error_label = ctk.CTkLabel(right_col_content, text=str(error))
            error_label.pack(anchor="w", padx=12, pady=(0, 10))

    create_month_buttons(top_row, on_month_click)
