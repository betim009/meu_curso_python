...
def filtrar_coluna(df, col, valor):
    new_df = []
    for i, row in df.iterrows():
        try:
            ...
            print(f"Error foi na linha: {i}", error)
            continue
    return pd.DataFrame(new_df)


def filtrar_coluna_null(df, col, valor):
    new_df = []
    for i, row in df.iterrows():
        try:
            if valor in row[col] or "-" in row[col]:
                new_df.append(row)
        except TypeError as error:
            print(f"Error foi na linha: {i}", error)
            continue
